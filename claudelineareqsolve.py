...
def solve_undetermined_coefficients(A, forcing, verbose=True):
    ...
    for force in forcing:
        ...
        if ftype == 'const':
            _, b = force
            _, Xp = solve_nonhomogeneous_constant(A, b, verbose=False)
            if any(not frac_is_zero(x) for x in Xp):
                Xp_parts.append(format_vector(Xp))
        elif ftype == 'exp':
            _, b, k = force
            _, Xp = solve_nonhomogeneous_exp(A, b, k, verbose=False)
            if isinstance(Xp, list):
                ks = format_frac(k)
                Xp_parts.append(f"{format_vector(Xp)}e^({ks}t)")
            else:
                Xp_parts.append(str(Xp))
        elif ftype == 'poly':
            _, poly_vecs = force
            _, pvecs = solve_nonhomogeneous_polynomial(A, poly_vecs, verbose=False)
            max_pw = max(poly_vecs, key=lambda x: x[0])[0]
            pp = []
            for pw in range(max_pw, -1, -1):
                if pw in pvecs and any(not frac_is_zero(x) for x in pvecs[pw]):
                    vs = format_vector(pvecs[pw])
                    if pw == 0:   pp.append(vs)
                    elif pw == 1: pp.append(f"{vs}t")
                    else:         pp.append(f"{vs}t^{pw}")
            if pp:
                Xp_parts.append(" + ".join(pp))
    ...
    if verbose:
        print_steps("Undetermined Coefficients — Full Solution", [
            f"Homogeneous: {hom_sol}",
            f"Full general solution:\n  {full}",
        ])
    return full
